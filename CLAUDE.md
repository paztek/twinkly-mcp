...
> **Current state: Phase 7 complete.** TypeScript toolchain + CI (Phase 0), the config layer (`src/config.ts`, 37 tests), the device manager (`src/twinkly/device-manager.ts` + `src/errors.ts`, 26 tests), the MCP bootstrap (`src/server.ts` + `src/index.ts` + `src/tools/` + `src/logger.ts`), the read tools (`src/twinkly/format.ts` + `src/tools/status.ts` + `discover_devices`), the control tools (`src/tools/power.ts` + `color.ts` + `effects.ts` + `movies.ts`), the safety layer (read-only / tool-group / admin gating via `src/tools/shared.ts` + gated `src/tools/device-admin.ts`), and the optional extras (Streamable HTTP transport `src/transport/http.ts` + gated realtime `src/tools/realtime.ts`) are in place — **131 tests, ~97% coverage**. The server runs over **stdio or Streamable HTTP** (`TWINKLY_TRANSPORT=http`, `/mcp` on loopback) and exposes the full everyday surface (`list_devices`, `discover_devices`, `get_device_details`, `get_summary`, `get_state`, `set_power`, `set_mode`, `set_color`, `set_brightness`, `set_saturation`, `list_effects`, `set_effect`, `list_movies`, `set_movie`) plus the gated admin tools (`set_name`, `set_timer`, realtime `send_frame`), all filterable by `TWINKLY_READONLY` / `TWINKLY_TOOLS` / `TWINKLY_ALLOW_ADMIN`. Only Phase 8 (docs + final test pass) remains. The plan below tracks the build — update the checkboxes as phases land.
