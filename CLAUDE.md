...
> **Current state: Phase 8 complete (functionally done; only Phase 9 npm-publishing remains).** TypeScript toolchain + CI (Phase 0), the config layer (`src/config.ts`, 37 tests), the device manager (`src/twinkly/device-manager.ts` + `src/errors.ts`, 26 tests), the MCP bootstrap (`src/server.ts` + `src/index.ts` + `src/tools/` + `src/logger.ts`), the read tools (`src/twinkly/format.ts` + `src/tools/status.ts` + `discover_devices`), the control tools (`src/tools/power.ts` + `color.ts` + `effects.ts` + `movies.ts`), the safety layer (read-only / tool-group / admin gating via `src/tools/shared.ts` + gated `src/tools/device-admin.ts`), the optional extras (Streamable HTTP transport `src/transport/http.ts` + gated realtime `src/tools/realtime.ts`), and the docs + guarded real-device smoke test (`src/integration.test.ts`, `README.md`) are in place — **137 tests (1 skipped without hardware), ~98% coverage**. The server runs over **stdio or Streamable HTTP** (`TWINKLY_TRANSPORT=http`, `/mcp` on loopback) and exposes the full everyday surface (`list_devices`, `discover_devices`, `get_device_details`, `get_summary`, `get_state`, `set_power`, `set_mode`, `set_color`, `set_brightness`, `set_saturation`, `list_effects`, `set_effect`, `list_movies`, `set_movie`) plus the gated admin tools (`set_name`, `set_timer`, realtime `send_frame`), all filterable by `TWINKLY_READONLY` / `TWINKLY_TOOLS` / `TWINKLY_ALLOW_ADMIN`. Only Phase 9 (npm publish / frictionless install) remains. The plan below tracks the build — update the checkboxes as phases land.
...
- [ ] **Phase 9 — Frictionless install ("as easy as Notion").** _Packaging prep landed; the actual `npm publish` (needs the maintainer's npm credentials + 2FA) and the registry listings are what remain._ Prep done: `LICENSE` (ISC) added; `repository`/`homepage`/`bugs`/`keywords` filled in for the npm page and registry scrapers; version bumped to `0.1.0` (**and `SERVER_VERSION` in `server.ts` with it**); `prepublishOnly` runs typecheck + tests + build so a stale `dist/` can't ship; `tsconfig.build.json` keeps `src/test-utils.ts` and the `*.test.ts` files out of the published tarball (`npm run build` uses it, `typecheck` still uses the root config so tests stay type-checked). Verified via `npm pack` + `npx ./twinkly-mcp-0.1.0.tgz`: 57 files / 39.5 kB, `LICENSE` + `README.md` included, no test-utils, bin runs and answers initialize (`version: 0.1.0`) + tools/list (14 tools) with clean stdout. README now leads with `claude mcp add twinkly --env TWINKLY_IP=… -- npx -y twinkly-mcp` and `.mcp.json`-style snippets (**keep `-y` on every documented `npx`** — without it npx can stall on an install prompt no MCP client can answer). Make adoption a copy-paste one-liner. Publish to npm so the server runs via `npx -y twinkly-mcp` (no clone/build) — verify the `bin` shebang + `dist/` ship correctly and `files`/`.npmignore` are scoped. Provide a single documented recipe, `claude mcp add twinkly -- npx -y twinkly-mcp`, plus ready-to-paste `.mcp.json` / `claude_desktop_config.json` snippets for the common clients (Claude Code, Claude Desktop, Cursor). List in discovery surfaces: the official MCP registry, awesome-mcp-servers, Smithery, etc. Optional `mcp-name:`/server manifest metadata so registries can auto-ingest. Smooth the device-config step (the one bit Notion's OAuth hides): zero-config UX via `TWINKLY_DISCOVERY` so a fresh install finds lights on the LAN with no env vars, and a clear first-run message when none are found. **Caveat: unlike Notion, this is a LAN device controller — a hosted/remote OAuth server doesn't fit (it must run on the user's network), so "easy" here means a zero-build local launch + auto-discovery, not a hosted endpoint.** **Milestone: install in under a minute from a single command.**
...
- **Firmware response shapes vary and the twinklyjs types under-describe them.** `/led/effects/current` is the known case: older firmware returns a numeric `effect_id`, family T (e.g. TWQ064STW) returns `preset_id` + the effect's `unique_id` and no `effect_id` at all — `formatEffects` accepts either and also surfaces `currentEffectUniqueId`. When a field comes back `undefined` against real hardware, widen the formatter's input type (`Partial<…> & { … }`) and fall back, rather than trusting the library's declared response type.
