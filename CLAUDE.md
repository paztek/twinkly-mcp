...
> **Current state: Phase 2 complete.** TypeScript toolchain + CI (Phase 0), the config layer (`src/config.ts`, 37 tests), and the device manager (`src/twinkly/device-manager.ts` + `src/errors.ts`, 26 tests) are in place; `src/index.ts` is still a placeholder bin. The plan below tracks the build — update the checkboxes as phases land.
